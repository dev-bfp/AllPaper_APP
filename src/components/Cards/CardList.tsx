...
export default function CardList() {
  ...
  const handleAddCard = (newCard: Omit<Card, 'id' | 'created_at'>) => {
    addCard(newCard);
  ...
  return (
    <>
                  ...
                  <div className="flex justify-between">
                    <div>
                      ...
                      <p className="font-medium">
                        {showBalances 
                          ? `R$ ${card.limit_amount?.toLocaleString('pt-BR')}` 
                          : '****'
                        }
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="font-medium">
                        {showBalances 
                          ? `R$ ${card.current_balance?.toLocaleString('pt-BR')}` 
                          : '****'
                        }
                      </p>
                    </div>
              ...
              <div className="space-y-4">
                ...
                {editingCard.type === 'credit' && (
                  <div>
                    ...
                    <input
                      type="number"
                      value={editingCard.limit_amount || ''}
                      onChange={(e) => setEditingCard({
                        ...editingCard,
                        limit_amount: parseFloat(e.target.value)
                      })}
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 dark:text-white"
                    />
                  </div>
      ...
      {/* Modal de Cadastro de Novo Cartão */}
      {showAddCardModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">
            ...
            <form onSubmit={(e) => {
              ...
              const newCard = {
                ...
                limit_amount: formData.get('type') === 'credit' ? parseFloat(formData.get('limit_amount') as string) : undefined,
                current_balance: 0,
